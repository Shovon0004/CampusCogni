...
export interface ExtendedCardFlipProps {
    title?: string;
    subtitle?: string;
    description?: string;
    features?: string[];
...
export default function ExtendedCardFlip({
    ...
    extraInfo = [],
    onClick,
}: ExtendedCardFlipProps) {
    const [isFlipped, setIsFlipped] = useState(false);
    
    // Handle click and flip state separately
    const handleClick = () => {
        if (onClick) onClick();
    };
                ...
                >
                    <div className="flex-1 space-y-3.5 overflow-y-auto">
                        ...
                        {(education || experience || location) && (
                            <div className="space-y-2 border-t border-zinc-200 dark:border-zinc-800/50 pt-2 mb-2">
                                {education && (
                                    <div className="flex items-start gap-2 text-sm">
                                        <div className="w-20 text-zinc-500 dark:text-zinc-400 flex-shrink-0">Education:</div>
                                        <div className="text-zinc-700 dark:text-zinc-300 line-clamp-1">{education}</div>
                                    </div>
                                )}
                                
                                {experience && (
                                    <div className="flex items-start gap-2 text-sm">
                                        <div className="w-20 text-zinc-500 dark:text-zinc-400 flex-shrink-0">Experience:</div>
                                        <div className="text-zinc-700 dark:text-zinc-300 line-clamp-1">{experience}</div>
                                    </div>
                                )}
                                
                                {location && (
                                    <div className="flex items-start gap-2 text-sm">
                                        <div className="w-20 text-zinc-500 dark:text-zinc-400 flex-shrink-0">Location:</div>
                                        <div className="text-zinc-700 dark:text-zinc-300 line-clamp-1">{location}</div>
                                    </div>
                                )}
                                
                                {extraInfo && extraInfo.length > 0 && extraInfo.map((info, idx) => (
                                    <div key={idx} className="flex items-start gap-2 text-sm">
                                        <div className="text-zinc-700 dark:text-zinc-300 line-clamp-2">{info}</div>
                                    </div>
                                ))}
                            </div>
                        )}
                        ...
                        <div className="space-y-1">
                            <div className="text-sm font-medium text-zinc-800 dark:text-zinc-200">Key Points</div>
                            <div className="space-y-1.5">
                                {features.map((feature, index) => (
                                    <div
                                        key={`${feature}-${index}`} // Use index in key for safety
                                        className="flex items-start gap-2 text-sm text-zinc-700 dark:text-zinc-300 transition-all duration-500"
                                        style={{
                                            transform: isFlipped
                                                ? "translateX(0)"
                                                : "translateX(-10px)",
                                    ...
                                    >
                                        <ArrowRight className="w-3 h-3 mt-1 text-orange-500 flex-shrink-0" />
                    ...
                    <div className="pt-3 mt-3 border-t border-zinc-200 dark:border-zinc-800 flex items-center justify-center">
                        <div
                            className={cn(
